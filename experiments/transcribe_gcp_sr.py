...
def process_audio(recognizer, audio, fn):
    print("[gcp] in thread: ", audio)
    text = recognizer.recognize_google_cloud(audio, language="en-US", show_all=True)
...
def get_callback(fn):
    def callback(recognizer, audio):
        try:
            print("[gcp] processing audio")
            voice_recognition_executor.submit(process_audio, recognizer, audio, fn)
        except sr.UnknownValueError:
            print("[gcp] could not understand audio")
        except sr.RequestError as e:
            print("[gcp] Could not request results from gcp; {0}".format(e))
...
if __name__ == "__main__":
    ...
    transcribe_gcp(handle_transcript)
