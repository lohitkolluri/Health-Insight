...
def connect(sid, env):
    print("connect ", sid)
    # Send a test message to the client when connected
    sio.emit(
        "test_connection",
...
def stop_recording(sid, value):
    ...
    if stop is not None:
        ...
        print("'stop' key not found in state_store.")
    return True
...
def generate_notes(sid, doctors_hints):
    ...
    notes = clinical_note_writer.run(
        {"input": doctors_hints, "transcript": state_store["transcript"]},
...
def patient_message(sid, text):
    ...
    callback = SocketIOCallback(
        lambda partial_ai_response: sio.emit(
            "patient_message", {"text": partial_ai_response, "done": False}, sid
        )
    )
    ...
    sio.emit(
        "patient_message", {"text": ai_response, "done": True, "audio": audio}, sid
    )
...
def transcript_callback(text):
    ...
    print(
        "[main] transcript callback. patient_mode:{}, patient_recording:{}".format(
            state_store["patient_mode"], state_store["patient_recording"]
        )
    )
    if state_store["patient_mode"]:
        ...
        with ThreadPoolExecutor(4) as e:
            e.submit(
                run_on_transcript, state_store["transcript"], send_cds_qa, cds_helper_qa
            )
            e.submit(
                run_on_transcript,
