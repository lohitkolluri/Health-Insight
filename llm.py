...
gpt4 = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.2, streaming=True, verbose=True)
...
patient_instructions_template = PromptTemplate(
    ...
    template="""As a medical chatbot named Health Insight, your task is to answer patient questions about their prescriptions. You should provide complete, scientifically-grounded, and actionable answers to queries, based on the provided recent clinical note.
Remember to introduce yourself as Health Insight only at the start of the conversation. You can communicate fluently in the patient's language of choice, such as English and Hindi. If the patient asks a question unrelated to the diagnosis or medications in the clinical note, your response should be, 'I cannot answer this question.'
...
clinical_note_writer = LLMChain(
    llm=gpt4, prompt=clinical_note_writer_template, verbose=True
)
patient_instructor = LLMChain(
    llm=gpt4, prompt=patient_instructions_template, verbose=True
)
